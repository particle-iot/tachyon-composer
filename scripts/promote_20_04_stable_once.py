...
ROWS = {(region, variant, "formfactor_dvt") for region in ("NA", "RoW")
        for variant in ("desktop", "headless", "manufacturing")}


def is_20(build):
    return build["distribution"] == "ubuntu" and build["distribution_version"] == "20.04"


def validate_matrix(builds, version):
    if len(builds) != 6 or {(b["region"], b["variant"], b["board"]) for b in builds} != ROWS:
        raise ValueError("Expected exactly NA/RoW x desktop/headless/manufacturing for formfactor_dvt")
    if any(not is_20(b) or b["version"] != version for b in builds):
        raise ValueError(f"Expected only Ubuntu 20.04 {version}")
...
def plan_promotion(stable, target):
    ...
    for build in target["builds"]:
        ...
        expected_url = ("https://linux-dist.particle.io/release/tachyon-ubuntu-20.04-"
                        f"{build['region']}-{build['variant']}-{build['board']}-{TARGET}.zip")
        if artifact["artifact_url"] != expected_url:
            raise ValueError("Artifact is not from the pinned published release")
    current = [b for b in stable["builds"] if is_20(b)]
    ...
    proposed["builds"] = [b for b in proposed["builds"] if not is_20(b)] + copy.deepcopy(target["builds"])
...
def main():
    ...
    target_path = output / "release-1.0.193.json"
    ...
    if proposed == before:
        ...
        print(f"Promoted Ubuntu 20.04 stable: {EXPECTED} -> {TARGET}")
    else:
